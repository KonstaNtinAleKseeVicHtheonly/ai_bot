...
from app.database.models import AI_type, AI_Model, Order
...
async def create_ai_model_2(ai_model_name: str, ai_type_name: str, ai_model_price:str):
    ...
    try:
        async with async_session() as session:
            ...
            check_type = await check_ai_type(session, ai_type_name)
            if not check_type:
...
async def create_ai_model(ai_model_name:str, ai_type_name:str):
    async with async_session() as session: 
        try:
            async with async_session() as session: 
                check_type = await check_ai_type(session,ai_type_name)
                if not check_type:
                    logger.warning("указанный вами тип ai не сущесвтует в базе")
                    return False
                new_ai_model = AI_Model(name=ai_model_name, ai_type=check_type.id)
                session.add(new_ai_model)
                await session.commit()
                logger.info("Новая ai модель успешно создана")
                return new_ai_model       
        except ValueError as err:
            # Ловим конкретное исключение из check_ai_type
            logger.error(f"❌ Ошибка проверки типа AI: {err}")
            return None
        except Exception as err:
            logger.error(f"❌ Ошибка создания модели AI '{ai_model_name}': {err}")
            await session.rollback()  # Важно откатить транзакцию при ошибке
            return None
        # existing_ai_type = await session.scalar(
        #         select(AI_type).where(AI_type.name == ai_type_name)) 
        # if not existing_ai_type:
        #         logger.info(f"ℹ️ Тип AI '{ai_type_name}' не существует")
        #         return False
        # new_ai_model = AI_Model(name=ai_model_name, ai_type=existing_ai_type.id)
        # session.add(new_ai_model)
        # await session.commit()
        # logger.info("Новая ai модель успешно создана")
        # return new_ai_model    
