...
async def set_dialog_mod(callback : CallbackQuery, state:FSMContext):
    ...
    if not current_user:
        ...
        await state.update_data(ai_type=text_ai_model_info.aimodel_type)
...
async def ai_text_chatting(message : Message, state : FSMContext):
    ...
    if Decimal(current_user_balance) <= 0:
        ...
        await state.set_state(ChatMode.text)
        await message.answer("Введите следующий запрос либо выйдите из режима диалога, нажав кнопку отмены",reply_markup=cancel_keyboard)
...
async def set_image_mod(callback : CallbackQuery, state:FSMContext):
    ...
    await state.update_data(ai_type=image_ai_model_info.aimodel_type)
...
async def set_vision_mod(callback : CallbackQuery, state:FSMContext):
    ...
    if not current_user:
        ...
        logger.warning(f"Выбрана модель для генерации текста по фото {text_ai_model_info.name} : {text_ai_model_info.aimodel_type}")
        await state.update_data(ai_model=text_ai_model_info.name)
        await state.update_data(ai_type=text_ai_model_info.aimodel_type)
...
@user_handler.message(~F.photo, StateFilter(ChatMode.vision))
async def wrong_image_vision_dialog(message : Message):
    await message.answer("Пожалуйста отправьте фотографию и описание к ней для успешной генерации")
